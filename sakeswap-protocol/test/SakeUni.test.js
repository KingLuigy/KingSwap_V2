...
contract('SakeUni', ([alice, bob, carol, jim, minter]) => {
    ...
    it('should set correct state variables', async () => {
        const sakeUni = await SakeUni.new(this.sake.address, this.uniToken.address, bob, carol, "50", `${this.testStartBlock}`, { from: alice });
        ...
        assert.equal((await sakeUni.startBlock()).valueOf(), `${this.testStartBlock}`);
        assert.equal((await sakeUni.endBlock()).valueOf(), `${this.testStartBlock + 128000}`);
        assert.equal((await sakeUni.bonusEndBlock()).valueOf(), `${this.testStartBlock + 64000}`);
    });
    ...
    it('getMultiplier', async () => {
        const sakeUni = await SakeUni.new(this.sake.address, this.uniToken.address, bob, carol, "50", `${this.testStartBlock}`, { from: alice });
        assert.equal((await sakeUni.getMultiplier(`${this.testStartBlock}`, `${this.testStartBlock + 64000}`)).valueOf(), '128000');
        assert.equal((await sakeUni.getMultiplier(`${this.testStartBlock + 32000}`, `${this.testStartBlock + 70000}`)).valueOf(), '70000');
        assert.equal((await sakeUni.getMultiplier(`${this.testStartBlock + 32000}`, `${this.testStartBlock + 130000}`)).valueOf(), '128000');
        assert.equal((await sakeUni.getMultiplier(`${this.testStartBlock + 64000}`, `${this.testStartBlock + 128000}`)).valueOf(), '64000');
        assert.equal((await sakeUni.getMultiplier(`${this.testStartBlock + 64000}`, `${this.testStartBlock + 130000}`)).valueOf(), '64000');
        assert.equal((await sakeUni.getMultiplier(`${this.testStartBlock + 128000}`, `${this.testStartBlock + 130000}`)).valueOf(), '0');
    });
    ...
    context('With ERC/LP token added to the field', () => {
        ...
        it('should give out SAKEs only after farming time', async () => {
            ...
            const sakeUni = await SakeUni.new(this.sake.address, this.uniToken.address, bob, carol, "50", `${this.testStartBlock + 100}`, { from: alice });
        ...
        it('should distribute SAKEs properly for each staker', async () => {
           ...
           const sakeUni = await SakeUni.new(this.sake.address, this.uniToken.address, bob, carol, "50", `${this.testStartBlock + 300}`, { from: alice });
            this.sake.mint(sakeUni.address, '100000000000000000000');
            await sakeUni.add('100', this.lp.address, this.uniStake.address, false);
        ...
        it('should give proper SAKEs allocation to each pool', async () => {
            ...
            const sakeUni = await SakeUni.new(this.sake.address, this.uniToken.address, bob, carol, "50", `${this.testStartBlock + 400}`, { from: alice });
            this.sake.mint(sakeUni.address, '100000000000000000000');
            await this.lp.approve(sakeUni.address, '1000', { from: alice });
        ...
        // it('earn UNIs', async () => {
        //     const sakeUni = await SakeUni.new(this.sake.address, this.uniToken.address, jim, carol, "50", `${this.testStartBlock}`, { from: alice });
        //     this.sake.mint(sakeUni.address, '100000000000000000000');
