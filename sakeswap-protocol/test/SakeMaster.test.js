...
contract('SakeMaster', ([alice, bob, carol, dev, minter]) => {
    ...
    it('should set correct state variables', async () => {
        this.chef = await SakeMaster.new(this.sake.address, dev, '1000', `${this.testStartBlock}`, { from: alice });
    ...
    context('With ERC/LP token added to the field', () => {
        ...
        it('should allow emergency withdraw', async () => {
            ...
            this.chef = await SakeMaster.new(this.sake.address, dev, '100', `${this.testStartBlock + 100}`, { from: alice });
        ...
        it('should give out SAKEs only after farming time', async () => {
            // 100 per block farming rate starting at block 100 with bonus until block 1000
            this.chef = await SakeMaster.new(this.sake.address, dev, '50', `${this.testStartBlock + 100}`, { from: alice });
            await this.sake.transferOwnership(this.chef.address, { from: alice });
            await this.chef.add('100', this.lp.address, true);
            await this.lp.approve(this.chef.address, '1000', { from: bob });
            await this.chef.deposit(0, '100', { from: bob });
        ...
        it('should not distribute SAKEs if no one deposit', async () => {
            ...
            this.chef = await SakeMaster.new(this.sake.address, dev, '50', `${this.testStartBlock + 200}`, { from: alice });
            await this.sake.transferOwnership(this.chef.address, { from: alice });
            await this.chef.add('100', this.lp.address, true);
            await this.lp.approve(this.chef.address, '1000', { from: bob });
        ...
        it('should distribute SAKEs properly for each staker', async () => {
            ...
            this.chef = await SakeMaster.new(this.sake.address, dev, '50', `${this.testStartBlock + 300}`, { from: alice });
            await this.sake.transferOwnership(this.chef.address, { from: alice });
            await this.chef.add('100', this.lp.address, true);
        ...
        it('should give proper SAKEs allocation to each pool', async () => {
            ...
            this.chef = await SakeMaster.new(this.sake.address, dev, '50', `${this.testStartBlock + 400}`, { from: alice });
            await this.sake.transferOwnership(this.chef.address, { from: alice });
            await this.lp.approve(this.chef.address, '1000', { from: alice });
        ...
        //     this.chef = await SakeMaster.new(this.sake.address, dev, '100', `${this.testStartBlock + 500}`, { from: alice });
        //     await this.sake.transferOwnership(this.chef.address, { from: alice });
        //     await this.lp.approve(this.chef.address, '1000', { from: alice });
        ...
        //     await time.advanceBlockTo(`${this.testStartBlock + 135505}`);
        ...
        it('getMultiplier', async () => {
            this.sake = await SakeMaster.new(this.sake.address, dev, '50', `${this.testStartBlock}`, { from: alice });
            assert.equal((await this.sake.getMultiplier(`${this.testStartBlock}`, `${this.testStartBlock + 35000}`)).toString(), '35000');
            assert.equal((await this.sake.getMultiplier(`${this.testStartBlock + 10000}`, `${this.testStartBlock + 135000}`)).toString(), '2025000');
            assert.equal((await this.sake.getMultiplier(`${this.testStartBlock + 35000}`, `${this.testStartBlock + 135000}`)).toString(), '2000000');
            assert.equal((await this.sake.getMultiplier(`${this.testStartBlock + 10000}`, `${this.testStartBlock + 235000}`)).toString(), '2225000');
            assert.equal((await this.sake.getMultiplier(`${this.testStartBlock + 45000}`, `${this.testStartBlock + 235000}`)).toString(), '2000000');
            assert.equal((await this.sake.getMultiplier(`${this.testStartBlock + 145000}`, `${this.testStartBlock + 235000}`)).toString(), '180000');
            assert.equal((await this.sake.getMultiplier(`${this.testStartBlock + 145000}`, `${this.testStartBlock + 235001}`)).toString(), '180000');
            assert.equal((await this.sake.getMultiplier(`${this.testStartBlock + 235001}`, `${this.testStartBlock + 235002}`)).toString(), '0');
        });
        ...
        it('handover the saketoken mintage right', async () => {
            this.master = await SakeMaster.new(this.sake.address, dev, '50', `${this.testStartBlock}`, { from: alice });
