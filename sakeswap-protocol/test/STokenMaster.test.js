...
contract('STokenMaster', ([alice, bob, carol, jim, minter]) => {
    beforeEach(async () => {
        ...
        this.sTokenMaster = await STokenMaster.new(this.sake.address, bob, carol, '200', '10', `${this.testStartBlock}`, `${this.testStartBlock + 300000}`, { from: alice });
    });
    ...
    context('With ERC/LP token added to the field', () => {
        ...
        it('should give out SAKEs only after farming time', async () => {
            const sTokenMaster = await STokenMaster.new(this.sake.address, bob, carol, '10', '10', `${this.testStartBlock + 100}`, `${this.testStartBlock + 150}`, { from: alice });
        ...
        it('should not distribute SAKEs if no one deposit', async () => {
            const sTokenMaster = await STokenMaster.new(this.sake.address, bob, carol, '10', '50', `${this.testStartBlock + 200}`, `${this.testStartBlock + 300}`, { from: alice });
        ...
        it('should distribute SAKEs properly for each staker', async () => {
            const sTokenMaster = await STokenMaster.new(this.sake.address, bob, carol, '10', '50', `${this.testStartBlock + 300}`, `${this.testStartBlock + 400}`, { from: alice });
            this.sake.mint(sTokenMaster.address, '10000');
            await sTokenMaster.add('100', this.lpToken.address, this.sToken.address, true, { from: bob });
        ...
        it('should give proper SAKEs allocation to each pool', async () => {
            const sTokenMaster = await STokenMaster.new(this.sake.address, bob, carol, '10', '50', `${this.testStartBlock + 400}`, `${this.testStartBlock + 500}`, { from: alice });
            this.sake.mint(sTokenMaster.address, '10000');
            await this.lpToken.approve(sTokenMaster.address, '1000', { from: alice });
        ...
        it('should stop giving SAKEs after the stoken mining is end', async () => {
            const sTokenMaster = await STokenMaster.new(this.sake.address, bob, carol, '10', '50', `${this.testStartBlock + 500}`, `${this.testStartBlock + 600}`, { from: alice });
            this.sake.mint(sTokenMaster.address, '10000');
            await this.lpToken.approve(sTokenMaster.address, '1000', { from: alice });
            await this.sToken.approve(sTokenMaster.address, '1000', { from: alice });
            await sTokenMaster.add('100', this.lpToken.address, this.sToken.address, true, { from: bob });
        ...
        it('sake owner is sTokenMaster', async () => {
            const sTokenMaster = await STokenMaster.new(this.sake.address, bob, carol, '10', '50', `${this.testStartBlock + 700}`, `${this.testStartBlock + 800}`, { from: alice });
        ...
        it('with draw', async () => {
            ...
            const sTokenMaster = await STokenMaster.new(this.sake.address, bob, jim, '10', '50', `${this.testStartBlock + 800}`, `${this.testStartBlock + 900}`, { from: alice });
        ...
        it('emergency with draw', async () => {
            const sTokenMaster = await STokenMaster.new(this.sake.address, bob, jim, '10', '50', `${this.testStartBlock + 950}`, `${this.testStartBlock + 1050}`, { from: alice });
            this.sake.mint(sTokenMaster.address, '20000');
            await this.lpToken.approve(sTokenMaster.address, '1000', { from: alice });
            await this.sToken.approve(sTokenMaster.address, '1000', { from: alice });
            await sTokenMaster.add('100', this.lpToken.address, this.sToken.address, true, { from: bob });
        ...
        //     const sTokenMaster = await STokenMaster.new(this.sake.address, bob, jim, '10', '50', `${this.testStartBlock + 1100}`, `${this.testStartBlock + 1300}`, { from: alice });
        //     this.sake.mint(sTokenMaster.address, '20000');
        //     await this.lpToken.approve(sTokenMaster.address, '1000', { from: alice });
        //     await this.sToken.approve(sTokenMaster.address, '1000', { from: alice });
        //     await this.lpToken.approve(sTokenMaster.address, '1000', { from: bob });
