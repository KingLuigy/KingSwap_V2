...
contract('SakeMasterV2', ([alice, bob, carol, admin, sakefee, sakeMaker, minter]) => {
    beforeEach(async () => {
        ...
        this.sakeMasterV2 = await SakeMasterV2.new(this.sake.address, admin, sakeMaker, sakefee, `${this.testStartBlock}`, { from: alice });
    });
    ...
    it('should set correct state variables', async () => {
        ...
        assert.equal(sake.toString(), this.sake.address);
        ...
        assert.equal(sakePerBlockYieldFarming.toString(), '5000000000000000000');
        assert.equal(sakePerBlockTradeMining.toString(), '10000000000000000000');
    ...
    it('set trade mining speed up end block', async () => {
        assert.equal((await this.sakeMasterV2.tradeMiningSpeedUpEndBlock()).valueOf(), `${this.testStartBlock + 192000}`);
    ...
    it('set yield farming II end block', async () => {
        assert.equal((await this.sakeMasterV2.yieldFarmingIIEndBlock()).valueOf(), `${this.testStartBlock + 1152000}`);
        await expectRevert(this.sakeMasterV2.setYieldFarmingIIEndBlock(`${this.testStartBlock + 1200000}`, { from: alice }), 'yf:Call must come from admin.');
        await this.sakeMasterV2.setYieldFarmingIIEndBlock(`${this.testStartBlock + 1200000}`, { from: admin });
        assert.equal((await this.sakeMasterV2.yieldFarmingIIEndBlock()).valueOf(), `${this.testStartBlock + 1200000}`);
    });

    it('set trade mining end block', async () => {
        assert.equal(await this.sakeMasterV2.tradeMiningEndBlock().valueOf(), `${this.testStartBlock + 2304000}`);
        await expectRevert(this.sakeMasterV2.setTradeMiningEndBlock(`${this.testStartBlock + 2500000}`, { from: alice }), 'tm:Call must come from admin.');
        await this.sakeMasterV2.setTradeMiningEndBlock(`${this.testStartBlock + 2500000}`, { from: admin });
        assert.equal(await this.sakeMasterV2.tradeMiningEndBlock().valueOf(), `${this.testStartBlock + 2500000}`);
    });
    ...
    it('getMultiplier', async () => {
        const result1 = await this.sakeMasterV2.getMultiplier(`${this.testStartBlock}`, `${this.testStartBlock + 192000}`);
        assert.equal(result1.multipY.valueOf(), '192000');
        assert.equal(result1.multipT.valueOf(), '384000');
        const result2 = await this.sakeMasterV2.getMultiplier(`${this.testStartBlock}`, `${this.testStartBlock + 193000}`);
        assert.equal(result2.multipY.valueOf(), '193000');
        assert.equal(result2.multipT.valueOf(), '385000');
        const result3 = await this.sakeMasterV2.getMultiplier(`${this.testStartBlock}`, `${this.testStartBlock + 1152000}`);
        assert.equal(result3.multipY.valueOf(), '1152000');
        assert.equal(result3.multipT.valueOf(), '1344000');
        const result4 = await this.sakeMasterV2.getMultiplier(`${this.testStartBlock}`, `${this.testStartBlock + 1200000}`);
        assert.equal(result4.multipY.valueOf(), '1152000');
        assert.equal(result4.multipT.valueOf(), '1392000');
        const result5 = await this.sakeMasterV2.getMultiplier(`${this.testStartBlock + 1200000}`, `${this.testStartBlock + 1500000}`);
        assert.equal(result5.multipY.valueOf(), '0');
        assert.equal(result5.multipT.valueOf(), '300000');
        const result6 = await this.sakeMasterV2.getMultiplier(`${this.testStartBlock + 1200000}`, `${this.testStartBlock + 2305000}`);
        assert.equal(result6.multipY.valueOf(), '0');
        assert.equal(result6.multipT.valueOf(), '1104000');
        const result7 = await this.sakeMasterV2.getMultiplier(`${this.testStartBlock + 2305000}`, `${this.testStartBlock + 2306000}`);
        assert.equal(result7.multipY.valueOf(), '0');
        assert.equal(result7.multipT.valueOf(), '0');
    });

    it('getSakePerBlock', async () => {
        assert.equal(await this.sakeMasterV2.getSakePerBlock(`${this.testStartBlock + 100}`).valueOf(), '25000000000000000000');
        assert.equal(await this.sakeMasterV2.getSakePerBlock(`${this.testStartBlock + 192000}`).valueOf(), '25000000000000000000');
        assert.equal(await this.sakeMasterV2.getSakePerBlock(`${this.testStartBlock + 193000}`).valueOf(), '15000000000000000000');
        assert.equal(await this.sakeMasterV2.getSakePerBlock(`${this.testStartBlock + 1152000}`).valueOf(), '15000000000000000000');
        assert.equal(await this.sakeMasterV2.getSakePerBlock(`${this.testStartBlock + 1155000}`).valueOf(), '10000000000000000000');
        assert.equal(await this.sakeMasterV2.getSakePerBlock(`${this.testStartBlock + 2304000}`).valueOf(), '10000000000000000000');
        assert.equal(await this.sakeMasterV2.getSakePerBlock(`${this.testStartBlock + 2305000}`).valueOf(), '0');
    });
    ...
    context('With ERC/LP token added to the field', () => {
        ...
        it('should give out SAKEs only after farming time', async () => {
            const sakeMasterV2 = await SakeMasterV2.new(this.sake.address, admin, sakeMaker, sakefee, `${this.testStartBlock + 200}`, { from: alice });
        ...
        it('should distribute SAKEs properly for each staker', async () => {
            ...
            const sakeMasterV2 = await SakeMasterV2.new(this.sake.address, admin, sakeMaker, sakefee, `${this.testStartBlock + 300}`, { from: alice });
        ...
        it('should give proper SAKEs allocation to each pool', async () => {
            const sakeMasterV2 = await SakeMasterV2.new(this.sake.address, admin, sakeMaker, sakefee, `${this.testStartBlock + 400}`, { from: alice });
            await this.sake.mint(sakeMasterV2.address, '5000');
            await this.sake.transferOwnership(sakeMasterV2.address, { from: alice });
            await sakeMasterV2.setSakePerBlockYieldFarming('5', false,{ from: admin });
            await sakeMasterV2.setSakePerBlockTradeMining('10', false,{ from: admin });
        ...
        it('should stop giving bonus SAKEs after the bonus period ends', async () => {
            const sakeMasterV2 = await SakeMasterV2.new(this.sake.address, admin, sakeMaker, sakefee, `${this.testStartBlock + 500}`, { from: alice });
            await this.sake.mint(sakeMasterV2.address, '5000');
            await this.sake.transferOwnership(sakeMasterV2.address, { from: alice });
            await sakeMasterV2.setSakePerBlockYieldFarming('5', false,{ from: admin });
            await sakeMasterV2.setSakePerBlockTradeMining('10', false,{ from: admin });
            ...
            await sakeMasterV2.setTradeMiningEndBlock(`${this.testStartBlock + 600}`, { from: admin });
            await sakeMasterV2.setYieldFarmingIIEndBlock(`${this.testStartBlock + 600}`, { from: admin });
        ...
        it('can not harvest sake if harvest interval less than withdraw interval', async () => {
            const sakeMasterV2 = await SakeMasterV2.new(this.sake.address, admin, sakeMaker, sakefee, `${this.testStartBlock + 650}`, { from: alice });
            await this.sake.mint(sakeMasterV2.address, '5000');
            await this.sake.transferOwnership(sakeMasterV2.address, { from: alice });
            await sakeMasterV2.setSakePerBlockYieldFarming('5', false,{ from: admin });
            await sakeMasterV2.setSakePerBlockTradeMining('10', false,{ from: admin });
        ...
        it('lp fee ratio', async () => {
            const sakeMasterV2 = await SakeMasterV2.new(this.sake.address, admin, sakeMaker, sakefee, `${this.testStartBlock + 750}`, { from: alice });
        ...
        it('sake fee ratio', async () => {
            const sakeMasterV2 = await SakeMasterV2.new(this.sake.address, admin, sakeMaker, sakefee, `${this.testStartBlock + 850}`, { from: alice });
            await this.sake.mint(sakeMasterV2.address, '5000');
            await this.sake.transferOwnership(sakeMasterV2.address, { from: alice });
            await sakeMasterV2.setSakePerBlockYieldFarming('5', false,{ from: admin });
            await sakeMasterV2.setSakePerBlockTradeMining('10', false,{ from: admin });
        ...
        it('withdraw', async () => {
            const sakeMasterV2 = await SakeMasterV2.new(this.sake.address, admin, sakeMaker, sakefee, `${this.testStartBlock + 1000}`, { from: alice });
            await this.sake.mint(sakeMasterV2.address, '5000');
            await this.sake.transferOwnership(sakeMasterV2.address, { from: alice });
            await sakeMasterV2.setSakePerBlockYieldFarming('5', false, { from: admin });
            await sakeMasterV2.setSakePerBlockTradeMining('10', false, { from: admin });
        ...
        it('emergency withdraw', async () => {
            const sakeMasterV2 = await SakeMasterV2.new(this.sake.address, admin, sakeMaker, sakefee, `${this.testStartBlock + 1100}`, { from: alice });
            await this.sake.mint(sakeMasterV2.address, '5000');
            await this.sake.transferOwnership(sakeMasterV2.address, { from: alice });
            await sakeMasterV2.setSakePerBlockYieldFarming('5', false, { from: admin });
            await sakeMasterV2.setSakePerBlockTradeMining('10', false, { from: admin });
