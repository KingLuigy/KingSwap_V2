...
contract('Archbishop', ([alice, bob, carol, stakeholder, minter]) => {
    before(async () => {
        this.startBlock = parseInt(await web3.eth.getBlockNumber());
    });
    ...
    it('should set correct state variables', async () => {
        this.bishop = await Archbishop
            .new(this.king.address, stakeholder, '1000', `${this.startBlock}`, { from: alice });
    ...
    context('With ERC/LP token added to the field', () => {
        ...
        it('should allow emergency withdraw', async () => {
            ...
            this.bishop = await Archbishop
                .new(this.king.address, stakeholder, '100', `${this.startBlock + '100'}`, { from: alice });
        ...
        it('should give out $KINGs only after farming time', async () => {
            // 100 per block farming rate starting at block 100 with bonus until block 1000
            this.bishop = await Archbishop
                .new(this.king.address, stakeholder, '1000', `${this.startBlock + '100'}`, { from: alice });
            await this.king.transferOwnership(this.bishop.address, { from: alice });
            await this.bishop.add('100', this.lp.address, true);
            await this.lp.approve(this.bishop.address, '1000', { from: bob });
            await this.bishop.deposit(0, '100', { from: bob });
            await time.advanceBlockTo(`${this.startBlock + '89'}`);
            ...
            await time.advanceBlockTo(`${this.startBlock + '109'}`);
        ...
        it('should not distribute $KINGs if no one deposit', async () => {
            ...
            this.bishop = await Archbishop
                .new(this.king.address, stakeholder, '1000', `${this.startBlock + '200'}`, { from: alice });
            await this.king.transferOwnership(this.bishop.address, { from: alice });
            await this.bishop.add('100', this.lp.address, true);
            await this.lp.approve(this.bishop.address, '1000', { from: bob });
        ...
        it('should distribute $KINGs properly for each staker', async () => {
            ...
            this.bishop = await Archbishop
                .new(this.king.address, stakeholder, '1000', `${this.startBlock + '300'}`, { from: alice });
            await this.king.transferOwnership(this.bishop.address, { from: alice });
            await this.bishop.add('100', this.lp.address, true);
            ...
            await time.advanceBlockTo(`${this.startBlock + '369'}`)
        ...
        it('should give proper $KINGs allocation to each pool', async () => {
            ...
            this.bishop = await Archbishop
                .new(this.king.address, stakeholder, '1000', `${this.startBlock + '400'}`, { from: alice });
            await this.king.transferOwnership(this.bishop.address, { from: alice });
            await this.lp.approve(this.bishop.address, '1000', { from: alice });
        ...
        xit('should stop giving bonus $KINGs after the bonus period ends', async () => {
            // TODO: Change the tested contract instantiation logic to make this test possible
            // ? Pass `BONUS_BLOCKNUM` as the constructor param (to set `bonusEndBlock` and `mintEndBlock`)
            // 100 per block farming rate starting at block 500 with bonus until block 600
            this.bishop = await Archbishop
                .new(this.king.address, stakeholder, '100', `${this.startBlock + '500'}`, { from: alice });
            await this.king.transferOwnership(this.bishop.address, { from: alice });
            await this.lp.approve(this.bishop.address, '1000', { from: alice });
            ...
            await time.advanceBlockTo(`${this.startBlock + '100505'}`);
        ...
        it('getMultiplier', async () => {
            this.bishop = await Archbishop
                .new(this.king.address, stakeholder, '1000', `${this.startBlock + '500'}`, { from: alice });
            assert.equal((await this.bishop.getMultiplier(`${this.startBlock + '500'}`, `${this.startBlock + '700'}`)).valueOf(), '2000');
            assert.equal((await this.bishop.getMultiplier(`${this.startBlock + '700'}`, `${this.startBlock + '701'}`)).valueOf(), '10');
            await time.advanceBlockTo(`${this.startBlock + '700'}`)
            assert.equal((await this.bishop.getMultiplier(`${this.startBlock + '700'}`, `${this.startBlock + '701'}`)).valueOf(), '10');
            await time.advanceBlockTo(`${this.startBlock + '800'}`)
            await expectRevert(this.bishop.changeBonusEndBlock(`${this.startBlock + '800'}`, { from: alice}), 'bonus end block has not ended');
            await expectRevert(this.bishop.changeMintEndBlock(`${this.startBlock + '800'}`, { from: alice}), 'mint end block has not ended');
            assert.equal((await this.bishop.getMultiplier(`${this.startBlock + '801'}`, `${this.startBlock + '802'}`)).valueOf(), '10');
        });

        it('transfer King Token Ownership', async () => {
            this.bishop = await Archbishop
                .new(this.king.address, stakeholder, '1000', `${this.startBlock }`, { from: alice });
            const oldOwner = await this.king.owner();
            await this.king.transferOwnership(this.bishop.address, { from: alice });
            const king = await this.bishop.king();
            const stakeholderaddress = await this.bishop.stakeholderaddress();
            const owner = await this.king.owner();
            assert.equal(king.valueOf(), this.king.address);
            assert.equal(stakeholderaddress.valueOf(), stakeholder);
            assert.equal(oldOwner.valueOf(), alice);
            assert.equal(owner.valueOf(), this.bishop.address);
        });

        it('add lp token', async () => {
            this.bishop = await Archbishop.new(this.king.address, stakeholder, '100', `${this.startBlock}`, { from: alice });
