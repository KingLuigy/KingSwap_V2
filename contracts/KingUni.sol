...
contract KingUni is Ownable {
    ...
    // Block number of distributing $KING period ends.
    ...
    function pending(uint256 _pid, address _user) external view returns (uint256 _king, uint256 _uni) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        _king = user.amount.mul(accKingPerShare).div(1e12).sub(user.kingRewardDebt);
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        uint256 pendingKing = user.amount.mul(pool.accKingPerShare).div(1e12).sub(user.kingRewardDebt);
        uint256 pendingUni = user.amount.mul(pool.accUniPerShare).div(1e12).sub(user.uniRewardDebt);
        user.amount = user.amount.add(_amount);
        user.kingRewardDebt = user.amount.mul(pool.accKingPerShare).div(1e12);
